...
int adventurerCard(int currentPlayer, int handPos, struct gameState *state){
	...
	while(drawntreasure<2 && (state->deckCount[currentPlayer] > 0 || state->discardCount[currentPlayer] > 0) && state->handCount[currentPlayer] < MAX_HAND)
    {
...
// original adventurerCard code below
// int adventurerCard(int currentPlayer, int handPos, struct gameState *state){
// 	int cardDrawn;
// 	int drawntreasure=0;
// 	int temphand[MAX_HAND];// moved above the if statement
